...
function getTile(slice::Dict{Point, Tile}, yBounds::Point, point::Point)
    (_, yMax) = yBounds
    (_, y) = point
    if y == yMax + 2
        return Rock()
...
function trickle(slice::Dict{Point, Tile}, previous::Continue, yBounds::Point)
    (x, y) = previous.point
    if blocked(getTile(slice, yBounds, previous.point))
        return OutOfBounds()
    end
    nextPoint = (x, y + 1)
    nextTile = getTile(slice, yBounds, nextPoint)
    if (!blocked(nextTile))
        return Continue(nextPoint)
    end
    nextPoint = (x - 1, y + 1)
    nextTile = getTile(slice, yBounds, nextPoint)
    if !blocked(nextTile)
...
function trickleSand(slice::Dict{Point, Tile})
    ...
    while trickling
        ...
        while current !== Done() && (trickling = current !== OutOfBounds())
            current = trickle(slice, current, yBounds)
        end
    end
    return slice
end
